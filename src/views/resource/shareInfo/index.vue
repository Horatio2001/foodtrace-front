<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogCollectionVisible" title="共享信息详情" :close-on-click-modal="true"
               :show-close="true"
...
import {
  ...
  fetchShareTotal,
  updateShare,
  loadInfo, fetchBlurShareListByPage
} from '@/api/shareInfo'
...
export default {
  ...
  methods: {
    ...
    getBlurList() {
      this.listLoading = true
      fetchBlurShareListByPage(this.listQuery.collectID).then(response => {
        this.list = response.data
        setTimeout(() => {
          this.listLoading = false
        }, 1.5 * 1000)
